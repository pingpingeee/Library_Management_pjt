...
<body>
	...
	<form id="frm">
		...
		<div class="form-group">
			...
			<div class="input-container">
				<textarea name="bookComent" required placeholder="책의 간략한 내용을 입력해주세요"></textarea>
			</div>
		</div>
		<br>
    ...
    document.getElementById('majorCategory').addEventListener('change', function() {
		...
		if (majorCategory === '000') {
			addSubCategories([ '010 도서관.서지학', '020 문헌정보학', '030 백과사전',
					'040 강연.수필.연설문집', '050 일반학회.단체.박물관', '060 일반전집',
					'070 신문.언론.저널리즘', '080 일반전집.총서', '090 향토자료' ]);
		} else if (majorCategory === '100') {
			addSubCategories([ '110 형이상학', '120 인식론.인과론.인간학', '130 세계',
					'140 경학', '150 동양철학.사상', '160 서양철학', '170 논리학', '180 윤리학',
					'190 윤리.도덕교육' ]);
		} else if (majorCategory === '200') {
			addSubCategories([ '210 비교종교', '220 불교', '230 기독교', '240 도교',
					'250 천도교', '260 신도', '270 힌두교.브라만교', '280 회교(이슬람교)',
					'290 기타 제종교' ]);
		} else if (majorCategory === '300') {
			addSubCategories([ '310 통계학', '320 경제학', '330 사회학.사회문제', '340 정치학',
					'350 행정학', '360 법학', '370 교육학', '380 풍속.민속학', '390 국방.군사학' ]);
		} else if (majorCategory === '400') {
			addSubCategories([ '410 수학', '420 물리학', '430 화학', '440 천문학',
					'450 지학', '460 생명과학', '470 식물학', '480 동물학', '490 기타 자연과학' ]);
		} else if (majorCategory === '500') {
			addSubCategories([ '510 의학', '520 일반공학.공학일반', '530 기계공학',
					'540 전기.전자공학', '550 건축공학', '560 화학공학', '570 제조업',
					'580 생활과학', '590 기타 기술과학' ]);
		} else if (majorCategory === '600') {
			addSubCategories([ '610 건축', '620 조각.조형예술', '630 회화', '640 서예',
					'650 사진.인쇄', '660 음악', '670 공연예술.매체예술', '680 오락.스포츠',
					'690 기타 예술' ]);
		} else if (majorCategory === '700') {
			addSubCategories([ '710 한국어', '720 중국어', '730 일본어', '740 영어',
					'750 독일어', '760 프랑스어', '770 스페인어', '780 기타 언어' ]);
		} else if (majorCategory === '800') {
			addSubCategories([ '810 한국문학', '820 중국문학', '830 일본문학', '840 영어문학',
					'850 독일문학', '860 프랑스문학', '870 스페인문학', '880 기타 문학' ]);
		} else if (majorCategory === '900') {
			addSubCategories([ '910 한국사', '920 동양사', '930 서양사', '940 역사이론',
					'950 지리학', '960 지도.여행', '970 문화사', '980 민속사', '990 기타 역사' ]);
		}
